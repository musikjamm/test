...
function PointBlankSniperDataCoreFrameMixin:OnEvent(event, ...)
  ...
  elseif event == "AUCTION_HOUSE_SHOW_COMMODITY_WON_NOTIFICATION" then
    _, itemLink, _, _, _, _, _,
    _, _, _, _, _, _, _,
    _, _, _ = GetItemInfo(PointBlankSniperDataCoreFrameMixin.lastPurchase["item_id"]) 
    price = PointBlankSniperDataCoreFrameMixin.lastPurchase["price"]
    quantity = PointBlankSniperDataCoreFrameMixin.lastPurchase["quantity"]
    local time_string = date("%H:%M:%S")
    if quantity > 1 then
      print(format("[%s] You won an auction for %sx%d for %s (%s each)", time_string, itemLink, quantity, GetMoneyString(price * quantity), GetMoneyString(price), "|cffffffff"))
    else
      print(format("[%s] You won an auction for %sx%d for %s", time_string, itemLink, quantity, GetMoneyString(price), "|cffffffff"))
    end
